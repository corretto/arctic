...
public final class TestDelayPreProcessor implements ArcticPlayerPreProcessor {
    private static final Logger log = LoggerFactory.getLogger(TestDelayPreProcessor.class);

    public static final String NAME = "testDelay";
    private static final int PRIORITY = 40;
    private final TimeController timeController;

    @Inject
    public TestDelayPreProcessor(final TimeController timeController) {
        this.timeController = timeController;
    }

    @Override
    public boolean preProcess(final ArcticRunningTest test) {
        timeController.waitFor(test.getRecording().getTimings().getStartDelayMs());
        return true;
    }
